...
function PopupPagamento({
  ...
  const handleConfirmar = async () => {
    if (!comprovante) {
      alert(
        "Por favor, anexe o comprovante antes de continuar."
      );
      return;
    }

    const idUsuarioSalvo =
      localStorage.getItem("usuario_id");

    if (!idUsuarioSalvo) {
      alert(
        "Erro crítico: O ID do usuário não foi encontrado na memória do navegador!"
      );
      return;
    }
    ...
    try {
      ...
      const resultadoEtapa1 =
        await respostaEtapa1.json();
