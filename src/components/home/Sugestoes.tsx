...
type TipoFiltro = "Todos" | "Pendentes" | "Aprovados" | "Fixados";
...
export default function Sugestoes({ modoAdmin = false }: SugestoesProps) {
  ...
  const dadosExibidos = listaSugestoes.filter((item) => {
    ...
    if (filtro === "Aprovados" || filtro === "Fixados") return item.status === "APROVADO";
    return true;
  });
  ...
  return (
    <div className="w-full max-w-[1800px] mx-auto px-2 sm:px-4">
      <div 
        className={`bg-[#C83D3D] w-full ${
          modoAdmin ? "min-h-[92vh] lg:min-h-[95vh]" : "min-h-[75vh] min-[360px]:min-h-[80vh]"
        } rounded-[15px] shadow-2xl px-3 min-[360px]:px-4 sm:px-6 md:px-10 relative overflow-hidden pb-20`}
      >
        ...
        <div className={`left-1/2 -translate-x-1/2 flex flex-col items-center w-full px-1 sm:px-4 relative ${modoAdmin ? "mt-[90px] sm:mt-[130px] md:mt-[160px] lg:mt-[180px]" : "mt-[170px] min-[360px]:mt-[210px] sm:mt-[240px] md:mt-[260px] lg:mt-[280px] xl:mt-[300px]"}`}>
            ...
            <div className="w-full max-w-[95%] sm:max-w-[800px] md:max-w-[1100px] lg:max-w-[1300px] flex items-center justify-between relative mb-6 mt-6 min-[360px]:mt-10 md:mt-16 xl:mt-10">
              <button onClick={() => setShowDropdown(!showDropdown)} className="flex items-center gap-2 sm:gap-5 md:gap-8 relative z-40">
                <div className="relative w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10">
                  <img 
                    src={filtroIcon} 
                    alt="filtro" 
                    className="relative top-[10px] sm:top-[15px] w-full h-full scale-[1.1] sm:scale-[1.3] object-contain md:translate-y-2" 
                  />
                </div>
              ...
              {showDropdown && (
                <div className="absolute top-10 sm:top-12 left-0 bg-white rounded-xl shadow-2xl overflow-hidden z-50 min-w-[150px] sm:min-w-[180px] md:translate-y-7">
                  {["Todos", "Pendentes", "Aprovados", "Fixados"].map((item) => (
                    <button 
                      key={item} 
                      onClick={() => { setFiltro(item as TipoFiltro); setShowDropdown(false); }} 
                      className="w-full px-4 sm:px-6 py-2 sm:py-3 text-left font-semibold text-[#101625] hover:bg-[#f1f1f1] transition-all text-sm sm:text-base"
                    >
                      {item}
                    </button>
                  ))}
          ...
          <div className={`${modoAdmin ? "bg-[#D86062]" : "bg-white"} rounded-[12px] shadow-md w-full max-w-[95%] sm:max-w-[800px] md:max-w-[1100px] lg:max-w-[1300px] mx-auto h-[320px] min-[360px]:h-[380px] sm:h-[420px] md:h-[340px] lg:h-[380px] xl:h-[420px] flex flex-col overflow-hidden relative`}>
            
            {modoAdmin && (
              <button 
                onClick={carregarSugestoes}
                disabled={loading}
                className="absolute top-4 right-5 sm:top-5 sm:right-7 z-30 p-2 text-white/80 hover:text-white hover:bg-white/10 active:scale-95 rounded-full transition-all duration-200 disabled:opacity-50"
            ...
            {modoAdmin ? (
              ...
              <textarea 
                ...
                placeholder={isSending ? "Enviando sua sugestão..." : "Digite sua sugestão aqui."} 
                className="w-full h-full px-4 sm:px-6 py-10 sm:py-12 md:py-16 resize-none outline-none text-center text-[#A8A8A8] font-semibold text-sm sm:text-base md:text-lg scroll-modern overflow-y-overlay md:overflow-y-auto disabled:bg-gray-50 disabled:cursor-not-allowed" 
              />
            )}
          </div>
          ...
          {!modoAdmin && (
            <button 
              ...
              className={`mt-5 sm:mt-8 bg-[#383636] text-white px-8 sm:px-14 py-2.5 sm:py-3 rounded-[10px] font-extrabold text-base sm:text-[1.6rem] transition ${isSending || !texto.trim() ? "opacity-60 cursor-not-allowed" : "hover:scale-105"}`}
            >
