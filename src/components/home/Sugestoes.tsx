        ...
        <div className="w-full max-w-[1800px] mx-auto min-h-[85vh] px-4 sm:px-6 md:px-10 overflow-x-hidden">
            <div className="bg-[#C83D3D] w-full min-h-[80vh] rounded-[15px] shadow-2xl px-4 sm:px-6 md:px-10 relative overflow-hidden pb-20">
                ...
                <div className={`left-1/2 -translate-x-1/2 flex flex-col items-center w-full px-4 relative ${modoAdmin ? "mt-[100px] sm:mt-[130px] md:mt-[160px] lg:mt-[180px]" : "mt-[210px] sm:mt-[240px] md:mt-[260px] lg:mt-[280px] xl:mt-[300px]"}`}>
                    ...
                    {modoAdmin && (
                        <div className="w-full max-w-[95%] sm:max-w-[800px] md:max-w-[1100px] lg:max-w-[1300px] mb-6 flex items-center justify-between">
                            <button 
                            ...
                            >
                                ...
                                <span className="text-white font-bold text-[1rem] sm:text-[1.1rem] md:text-xl">{filtro}</span>
                            </button>

                            <button
                                onClick={carregarSugestoes}
                                className="bg-white text-[#C83D3D] rounded-full h-[38px] sm:h-[40px] md:h-[42px] px-4 sm:px-6 md:px-8 shadow-lg font-bold hover:bg-[#b03535] transition-all text-[0.9rem] sm:text-[1rem] md:text-lg"
                            >
                                Atualizar
                            </button>
                        </div>
                    ...
                    <div className={`${modoAdmin ? "bg-[#D86062]" : "bg-white"} rounded-[12px] shadow-md w-full max-w-[95%] sm:max-w-[800px] md:max-w-[1100px] lg:max-w-[1300px] mx-auto h-[380px] sm:h-[420px] md:h-[340px] lg:h-[380px] xl:h-[420px] flex flex-col overflow-hidden relative`}>
                        ...
                        {modoAdmin ? (
                            <div className="flex-1 overflow-y-auto scroll-modern px-4 py-6">
                                {loading ? (
                                    <div className="text-center py-20 text-white/60">Carregando sugestões...</div>
                                ) : error ? (
                                    <div className="flex flex-col items-center justify-center py-20 text-center">
                                        <p className="text-red-300 mb-4">{error}</p>
                                        <button 
                                            onClick={carregarSugestoes}
                                            className="bg-white text-[#C83D3D] px-6 py-2.5 rounded-full font-bold hover:bg-gray-100"
                                        >
                                            Tentar novamente
                                        </button>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-center h-full py-24">
                                        <p className="text-white opacity-70 font-semibold">Nenhuma sugestão encontrada.</p>
                                    </div>
                                )}
                            </div>
                        ) : (
