...
export default function Pontuacao({ pontos, progresso, loading, usuario }: PontuacaoProps) {
  return (
        ...
        {loading ? (
          ...
          <>
            <div className="flex flex-col items-center gap-4 sm:gap-6 md:gap-8 lg:gap-10">
              <BarraPontos progresso={progresso} premium={usuario?.premium} />
              <PorcoPorcentagem progresso={progresso} />
            </div>
            <Moeda total={pontos} premium={usuario?.premium} />
          </>
        )}
      </div>
    </div>
