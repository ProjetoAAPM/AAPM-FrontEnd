...
function EscolhaPlano() {
    ...
    const handleConfirmarEnvio = async () => {
        ...
        try {
            ...
            const resultadoEtapa1 = await pagamentoUserService.gerarPagamento({
                usuario_id: idUsuarioSalvo ? Number(idUsuarioSalvo) : null,
                plano: planoSelecionado,
                valor: planoSelecionado === "premium" ? 100.00 : 50.00
            });

            const idPagamentoRaw = resultadoEtapa1.id_pagamento || resultadoEtapa1.id;

            if (!idPagamentoRaw) {
                alert("Erro: O servidor backend não retornou um ID de pagamento válido.");
                fecharModal();
                return;
            }

            // LÓGICA DELA ADAPTADA: Gera a URL temporária local
            const urlImagemLocal = URL.createObjectURL(comprovante);

            // ETAPA 2: Envia usando seu serviço com JSON puro
            await pagamentoUserService.enviarComprovante(
                Number(idPagamentoRaw),
                urlImagemLocal // Passando a URL local aqui
            );
            ...
            alert(erro.message || "Falha na operação de pagamento.");
        } finally {
            setLoading(false);
        }
    };
