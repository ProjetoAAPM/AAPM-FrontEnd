...
function EscolhaPlano() {
    ...
    const handleConfirmarEnvio = async () => {
        ...
        try {
            // GERAR PAGAMENTO
            const resultadoEtapa1 = await pagamentoUserService.gerarPagamento(dadosPagamento);
            const idPagamento = resultadoEtapa1.id_pagamento;

            // ENVIAR IMAGEM PARA SUPABASE
            ...
            if (error) {
                console.error(error);
                alert("Erro ao enviar comprovante para o Supabase.");
                return;
            }
            ...
            const resultadoEtapa2 = await pagamentoUserService.enviarComprovante({
            ...
            alert(erro.message || "Não foi possível concluir o envio.");
            fecharModal();
        }
    };
                ...
                >
                    ...
                    >
                        <input
                            type="file"
                            className="hidden"
                            onChange={(e) => setComprovanteComum(e.target.files?.[0] || null)}
                        />
                ...
                >
                    ...
                    >
                        <input
                            type="file"
                            className="hidden"
                            onChange={(e) => setComprovantePremium(e.target.files?.[0] || null)}
                        />
                        <p className="text-base font-semibold text-[#888888]">
